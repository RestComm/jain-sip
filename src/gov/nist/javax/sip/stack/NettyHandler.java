...
public class NettyHandler {
    ...
    private static StackLogger logger = CommonLogger.getLogger(NettyHandler.class);
    ...
    protected NettyHandler(SIPTransactionStack sipStack, NettyTcpMessageProcessor messageProcessor) {
    ...
    public SocketChannel sendBytes(InetAddress senderAddress,
        ...
        try {
            ...
            while (retry_count < max_retry) {
                if (clientSock != null && (!clientSock.isActive() || !clientSock.isOpen())) {
                    removeSocket(key);
                    clientSock = null;
                    newSocket = true;
                }
    ...
    public void closeAll() {
        ...
        for (Enumeration<SocketChannel> values = socketTable.elements(); values
                .hasMoreElements();) {
            NioSocketChannel s = (NioSocketChannel) values.nextElement();
            s.close();
        }
    ...
    public void stop() {
        try {
            ...
            Iterator<Entry<SocketChannel, NettyTcpMessageChannel>> entriesIterator = NettyTcpMessageChannel.channelMap.entrySet().iterator();
            while (entriesIterator.hasNext()) {
                Entry<SocketChannel, NettyTcpMessageChannel> entry = entriesIterator.next();
                SocketChannel socketChannel = entry.getKey();
                NettyTcpMessageChannel messageChannel = entry.getValue();
                ...
                entriesIterator = NettyTcpMessageChannel.channelMap.entrySet().iterator();
            }
        } catch (Exception e) {
    ...
    public SocketChannel createOrReuseSocket(InetAddress inetAddress, int port) throws IOException {
        String key = NettyHandler.makeKey(inetAddress, port);
        ...
        try {
            ...
            if (channel == null) { // this is where the threads will race
                ...
                if (channel != null && channel.isActive()) {
                    putSocket(NettyHandler.makeKey(inetAddress, port), channel);
                    if (logger.isLoggingEnabled(LogWriter.TRACE_DEBUG)) {
