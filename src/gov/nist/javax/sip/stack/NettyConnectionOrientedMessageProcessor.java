 ...
 *
 */
public abstract class NettyConnectionOrientedMessageProcessor extends MessageProcessor {

	private static StackLogger logger = CommonLogger.getLogger(NettyConnectionOrientedMessageProcessor.class);
	
	protected int nConnections;

    protected boolean isRunning;

    protected final Map<String, NettyConnectionOrientedMessageChannel> messageChannels;

    protected final Map<String, NettyConnectionOrientedMessageChannel> incomingMessageChannels;

    protected ServerSocket sock;

    protected int useCount;
	
	public NettyConnectionOrientedMessageProcessor(InetAddress ipAddress, int port,
			String transport, SIPTransactionStack sipStack) {
		super(ipAddress, port, transport, sipStack);
		
	    this.sipStack = sipStack;
	    
		this.messageChannels = new ConcurrentHashMap <String, NettyConnectionOrientedMessageChannel>();
        this.incomingMessageChannels = new ConcurrentHashMap <String, NettyConnectionOrientedMessageChannel>();
	}
    ...
    protected synchronized void remove(NettyConnectionOrientedMessageChannel messageChannel) {
    ...
    protected synchronized void cacheMessageChannel(NettyConnectionOrientedMessageChannel messageChannel) {
        String key = messageChannel.getKey();
        NettyConnectionOrientedMessageChannel currentChannel = messageChannels.get(key);
    ...
    public boolean closeReliableConnection(String peerAddress, int peerPort) throws IllegalArgumentException {
        ...
        synchronized (this) {
            NettyConnectionOrientedMessageChannel foundMessageChannel = messageChannels.get(messageChannelKey);
