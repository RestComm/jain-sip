...
public class NIOHandler {
    ...
    private SocketChannel openOutgoingConnection(InetAddress senderAddress,
            ...
            byte[] bytes, boolean isClient, NioTcpMessageChannel messageChannel, String key) throws IOException {
        int retry_count = 0;
    ...
    public SocketChannel sendBytes(InetAddress senderAddress,
        ...
        if(clientSock != null && (!clientSock.isConnected() || !clientSock.isOpen()) ) {
                newSocket = true;
                clientSock = openOutgoingConnection(senderAddress, receiverAddress, contactPort, transport, bytes, isClient, messageChannel, key);
        }
